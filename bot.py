...
async def check_username(username: str) -> tuple:
    """
    Uses the real Roblox user lookup API.
    If the username doesn't exist as a user -> it's available.
    Returns (is_available: bool, status: str, error: str|None)
    """
    try:
        loop = asyncio.get_event_loop()
        def _req():
            return requests.get(
                f"https://api.roblox.com/users/get-by-username",
        ...
        if r.status_code == 200:
            data = r.json()
            # If Id is 0 or errorMessage present -> username not taken
            if data.get("Id", 0) == 0 or "errorMessage" in data:
...
async def find_available(length: int, max_results: int = 10) -> tuple:
    """
    Returns (found: list, checked: int, errors: int, last_status: str)
    """
    found = []
    candidates = generate_candidates(length, count=600)
    checked = 0
    errors = 0
    last_status = "unknown"
    ...
    for name in candidates:
        if len(found) >= max_results:
            break
        available, status, err = await check_username(name)
        checked += 1
        last_status = status
        if err:
            errors += 1
            ...
            found.append(name)
        await asyncio.sleep(0.5)  # faster since this endpoint is more lenient

    return found, checked, errors, last_status
...
class LengthSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            f"🔍 Searching for **{length}-character** usernames... Check your DMs shortly!",
            ephemeral=True
        )

        found, checked, errors, last_status = await find_available(length, max_results=10)
        ...
        if found:
            ...
            result_lines = (
                ...
                f"Last status: `{last_status}` | Errors: `{errors}`\n\n"
                "This likely means all checked combos are taken. Try again for a new random batch!"
            )
            color = 0xED4245
        ...
        result_embed.set_footer(text="Claim these fast — they won't last!")

        dm_sent = await dm_user(member, result_embed)

        # ── Private temp channel ──
        ...
        try:
            ...
            ch_embed.add_field(name="Errors", value=str(errors), inline=True)
            ...
            await private_channel.send(f"{member.mention} • {dm_status}", embed=ch_embed)
...
async def api_debug(ctx):
    """Test the real Roblox availability API and DM the result."""
    # Test with a known taken name and a random garbage name unlikely to exist
    test_cases = [
        ("Roblox", "should be TAKEN"),
        ("xkq9z2", "probably available"),
        ("zz9", "probably available"),
    ]
    lines = []
    ...
    for name, hint in test_cases:
        available, status, err = await check_username(name)
        if err:
            lines.append(f"`{name}` ({hint}) → ❌ Error: `{err}`")
        else:
            flag = "✅ Available" if available else "❌ Taken"
            lines.append(f"`{name}` ({hint}) → {flag} (status: `{status}`)")
        await asyncio.sleep(0.6)

    description = (
        "\n".join(lines) + "\n\n"
        "If `Roblox` shows as **Taken** and `xkq9z2` shows as **Available**, the API is working correctly!"
    )
...
@is_owner_or_admin()
async def check_single(ctx, username: str):
    """Manually check a single username. Usage: !check abc"""
    available, status, err = await check_username(username)
    if err:
        await ctx.send(f"`{username}` → ❌ Error: `{err}`")
    elif available:
        await ctx.send(f"`{username}` → ✅ **Available!**")
    else:
        await ctx.send(f"`{username}` → ❌ Taken (status: `{status}`)")
...
async def bot_status(ctx):
    ...
    embed.add_field(name="Cooldowns Tracked", value=str(len(user_cooldowns)), inline=True)
    await ctx.send(embed=embed)
